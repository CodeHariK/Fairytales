...
const UnaryExample: FC = () => {
	...
	const handleSubmit = useCallback(
		async (e: FormEvent) => {
			e.preventDefault()
			setChatMessages((resp) => [...resp, { text: inputValue, sender: "user" }])
			setInputValue("")
			const response = await elizaClient.say({
				sentence: inputValue,
			})

			setChatMessages((resp) => [...resp, { text: response.sentence, sender: "eliza" }])
		},
		[inputValue]
	)
		...
		<div>
			<header className={styles.appHeader}>
				...
				<div className={styles.links}>
					...
					<Link href="/connect-query-test">Query Calls</Link>
				...
				{chatMessages.map((resp, i) => {
					return (
						<div
							key={i}
							className={
								resp.sender === "eliza" ? styles.elizaRespContainer : styles.userRespContainer
							}
						>
