...
export async function sendOrganizationInviteEmail({
	...
	await sendEmail({
      ...
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <a href="${env.NEXT_PUBLIC_BASE_URL}/organizations/invites/${invitation.id}" style="background-color: #28a745; color: white; padding: 12px 24px; text-decoration: none; border-radius: 4px; display: inline-block; margin: 16px 0;">Manage Invitation</a>
        <p>Best regards,<br>Your App Team</p>
      </div>
    `,
		text: `You're invited to join the ${organization.name} organization\n\nHello ${inviter.name},\n\n${inviter.name} invited you to join the ${organization.name} organization. Please click the link below to accept/reject the invitation:\n\n${env.NEXT_PUBLIC_BASE_URL}/organizations/invites/${invitation.id}\n\nBest regards,\nYour App Team`,
	})
}
